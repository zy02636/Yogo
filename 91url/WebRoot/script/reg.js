...
var checkUserName = function(){
	...
	var url = "checkUserName.action?username="+userName.value;
	url = encodeURI(url);
	xmlHttp.open("POST", url, true);
	xmlHttp.onreadystatechange = afterCheckUserName;
	xmlHttp.send(null);
}
var afterCheckUserName = function(){
	if (xmlHttp.readyState == 4) {
		if (xmlHttp.status == 200){
			...
			if(result == "SUCCESS"){
				...
				var checkUserNameInfo = document.getElementById("checkUserName");
				checkUserNameInfo.innerHTML = "<font style='color:red;font-size:12px;'>用户名已经存在</font>";
				submitUserName = false;
			}
		}
	}
...
var submitForm = function(){
	if(submitCode && submitUserName && submitPwd && submitRePwd){
		submitRegInfo();
	}else{
		var checkReg = document.getElementById("checkReg");
		checkReg.innerHTML = "<font style='color:red;font-size:12px;'>请检查注册信息</font>";
	}
}

var submitRegInfo = function(){
	...
	createXmlHttpRequest();
	var url = "preAddUser.action?username="+username+"&password="+pwd+"&rePassword="+rePwd+"&code="+code;
	url = encodeURI(url);
	xmlHttp.open("POST", url, true);
	xmlHttp.onreadystatechange = afterSubmitRegInfo;
	xmlHttp.send(null);
}
var afterSubmitRegInfo = function(){
	if (xmlHttp.readyState == 4) {
		if (xmlHttp.status == 200){
			var result = xmlHttp.responseText.trim();
			if(result == "FAIL"){
				var info = result.split("&&");
				var checkUserNameInfo = document.getElementById("checkUserName");//1
				var checkRePwd = document.getElementById("checkRePwd");//2
				var checkCode = document.getElementById("checkCode");//3
				
				var posInfo = info[1].trim();
				if(posInfo == "1"){
					var checkUserNameInfo = document.getElementById("checkUserName");
					checkUserNameInfo.innerHTML = info[0];
				}else if(posInfo == "2"){
					var checkRePwd = document.getElementById("checkRePwd");
					checkRePwd.innerHTML = info[0];
				}else if(posInfo == "3"){
					var checkCode = document.getElementById("checkCode");
					checkCode.innerHTML = info[0];
				}else{
					var checkReg = document.getElementById("checkReg");
					checkReg.innerHTML = info[0];
				}
			}else{
